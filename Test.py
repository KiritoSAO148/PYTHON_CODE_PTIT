...
def plot_freq_res_window(N):
    # N = 100  # Số mẫu của cửa sổ chữ nhật
    w = np.linspace(-np.pi, np.pi, 1000)  # Tần số góc

    # Tính hàm phản hồi biên độ của cửa sổ chữ nhật
    A = (np.sin(w * N / 2)) / (N * np.sin(w / 2))

    # Chuyển đổi sang đơn vị độ dB
    AdB = 20 * np.log10(np.abs(A))

    # Vẽ đồ thị
    plt.plot(w, AdB)
    plt.title('Đáp ứng biên độ của cửa sổ chữ nhật')
    plt.xlabel('Tần số góc (rad)')
    plt.ylabel('Đáp ứng biên độ (dB)')
    plt.grid(True)
    plt.show()

def plot_freq_res2_window(w, N, title):
    A = scipy.fftpack.fft()
